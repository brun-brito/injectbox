...
const InstanciasPage = ({ cliente }: Props) => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      const auth = localStorage.getItem(`zcampanha_auth_${cliente}`);
      if (auth !== 'true') {
        router.replace(`/zcampanha/${cliente}`);
        return;
      }
    }
    // Busca instâncias
                      ...
                      return (
                        <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <button
                            ...
                            onClick={() => router.push(`/zcampanha/${cliente}/instancias/${inst.idInstancia}`)}
                          >
